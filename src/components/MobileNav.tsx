              ...
              if (item.hasDropdown) {
                return (
                  <li key={index}>
                    ...
                    >
                      <CollapsibleTrigger className="flex items-center justify-between w-full px-4 py-3 text-sm font-semibold text-foreground hover:bg-muted rounded-lg transition-colors">
                        {item.label}
                        <ChevronDown
                          className={`h-4 w-4 transition-transform ${
                            (item.label === "HEARING HEALTH" && healthOpen) ||
                            (item.label === "SHOP" && shopOpen)
                              ? "rotate-180"
                              : ""
                          }`}
                        />
